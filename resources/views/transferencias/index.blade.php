...
@section('title', 'Transferências e Remanejamentos')
        ...
        <table class="rhc-table">
            ...
            <tbody>
                @forelse($remanejamentos as $rem)
                    ...
                    @endphp
                    <tr class="{{ $rowClass }}">
                        <td style="font-size:0.875rem; font-weight:500;">{{ $rem->item->nome ?? 'Item Deletado' }}</td>
                        <td style="font-family:ui-monospace,monospace; font-size:0.8rem; color:var(--slate-600);">{{ $rem->item->codigo ?? '-' }}</td>
                        <td style="text-align:center; font-weight:700; font-size:0.875rem;">{{ $rem->quantidade }}</td>
                        <td>
                            @if($rem->pedidoItemOrigem && $rem->pedidoItemOrigem->pedido)
                                <a href="{{ route('pedidos.show', $rem->pedidoItemOrigem->pedido->id) }}" class="pedido-link">
                                    #{{ $rem->pedidoItemOrigem->pedido->numero_pedido ?? 'S/N' }}
                                </a>
                                <div style="font-size:0.7rem; color:var(--slate-400); margin-top:0.125rem;">
                                    <i class="fas fa-building" style="font-size:0.6rem; margin-right:0.25rem;"></i>{{ $rem->pedidoItemOrigem->pedido->unidade->nome ?? '-' }}
                                </div>
                            @else
                        ...
                        <td>
                            @if($rem->pedidoDestino)
                                <a href="{{ route('pedidos.show', $rem->pedidoDestino->id) }}" class="pedido-link">
                                    #{{ $rem->pedidoDestino->numero_pedido ?? 'S/N' }}
                                </a>
                                <div style="font-size:0.7rem; color:var(--slate-400); margin-top:0.125rem;">
                                    <i class="fas fa-building" style="font-size:0.6rem; margin-right:0.25rem;"></i>{{ $rem->pedidoDestino->unidade->nome ?? '-' }}
                                </div>
                            @else
                                <span style="color:var(--slate-400); font-size:0.8rem;">Desconhecido</span>
                            @endif
                        ...
                        <td style="text-align:center;">
                            @if(!$isConcluido && $isAdminOrComprador)
                                <form action="{{ route('transferencias.confirmar', $rem->id) }}" method="POST" style="display:inline-flex; align-items:center; gap:0.375rem;">
                                    @csrf
                                    <input type="number" name="quantidade_recebida" class="qty-input" min="0" max="{{ $rem->quantidade }}" value="{{ $qtdRecebida }}">
                                    <button type="submit" class="confirm-btn confirm-btn-primary">
                                        <i class="fas fa-check" style="font-size:0.6rem;"></i> Confirmar
                                    </button>
                                </form>
                            @else
                                <span style="font-weight:700; font-size:0.875rem; color:{{ $isConcluido ? '#166534' : 'var(--slate-600)' }};">{{ $qtdRecebida }}</span>
                            @endif
                        </td>
                        <td>
                            <span class="transfer-status {{ $statusClass }}">
                                <i class="fas {{ $isConcluido ? 'fa-circle-check' : ($isTransito ? 'fa-truck' : 'fa-hourglass-half') }}" style="font-size:0.6rem;"></i>
                                {{ $statusLabel }}
                            </span>
                        </td>
                        <td style="font-size:0.75rem; color:var(--slate-400);">{{ $rem->created_at->format('d/m/Y H:i') }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="8">
                        ...
                        <div class="origin-card" style="background:white;">
                            ...
                            <div style="margin-bottom:0.75rem;">
                                ...
                                <select id="pedidoOrigemSelect" class="rhc-select tomselect" style="width:100%;" required>
                                    ...
                                    @foreach($pedidosOrigem as $p)
                                        <option value="{{ $p->id }}" data-itens="{{ json_encode($p->itens) }}">
                                            #{{ $p->numero_pedido ?? 'N/A' }} - {{ $p->unidade->nome ?? '-' }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
