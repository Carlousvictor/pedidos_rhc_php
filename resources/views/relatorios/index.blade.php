...
@section('title', 'Relatórios e KPIs')
...
{{-- Filters --}}
<form method="GET" action="{{ route('relatorios') }}" class="filter-card">
    <div style="display:flex; align-items:center; gap:0.5rem; margin-bottom:0.75rem;">
        ...
        @if(request('unidade_id') || request('data_inicio') || request('data_fim'))
            <a href="{{ route('relatorios') }}" style="margin-left:auto; font-size:0.6875rem; color:var(--red-500); text-decoration:none; font-weight:500; display:flex; align-items:center; gap:0.25rem;">
    ...
    <div style="display:grid; grid-template-columns:repeat(3, 1fr); gap:0.75rem;">
        ...
        <div>
            ...
            <input type="date" name="data_inicio" class="rhc-input" value="{{ request('data_inicio') }}" onchange="this.form.submit()">
        </div>
        <div>
            <div class="filter-label">Até</div>
            <input type="date" name="data_fim" class="rhc-input" value="{{ request('data_fim') }}" onchange="this.form.submit()">
        </div>
    </div>
</form>
...
<div class="kpi-grid">
    <div class="kpi-total">
        ...
        <div class="kpi-number">{{ $totalPedidos }}</div>
        ...
        <div style="display:flex; gap:0.25rem; margin-top:1rem;">
            @foreach($statusCards as $sc)
                <div style="flex:1; height:0.25rem; border-radius:9999px; background:rgba(255,255,255,0.2); overflow:hidden;">
                    <div style="height:100%; border-radius:9999px; background:rgba(255,255,255,0.7); width:{{ $totalPedidos ? round(($sc['count'] / $totalPedidos) * 100) : 0 }}%;"></div>
                </div>
            @endforeach
        </div>
    </div>
    ...
    @foreach($statusCards as $sc)
        @php $pct = $totalPedidos > 0 ? round(($sc['count'] / $totalPedidos) * 100) : 0; @endphp
        <div class="kpi-card">
            <div class="kpi-header">
                <div class="kpi-icon" style="background:{{ $sc['bg'] }}; color:{{ $sc['text'] }};">
                    <i class="fas {{ $sc['icon'] }}"></i>
                </div>
                <span class="kpi-value" style="color:{{ $sc['text'] }};">{{ $sc['count'] }}</span>
            </div>
            <div class="kpi-name">{{ $sc['label'] }}</div>
            <div class="kpi-bar">
                <div class="kpi-bar-fill" style="width:{{ $pct }}%; background:{{ $sc['color'] }};"></div>
            </div>
            <div class="kpi-bar-pct">{{ $pct }}% do total</div>
        </div>
    @endforeach
</div>

{{-- Funil + Remanejamentos --}}
        ...
        <div style="padding:0 1.25rem 1.25rem;">
                ...
                <div class="funnel-step">
                    <div class="funnel-row">
                        <div class="funnel-label">
                            ...
                            <span class="funnel-desc">{{ $step['desc'] }}</span>
                        </div>
                        <span class="funnel-value">{{ number_format($step['value'], 0, ',', '.') }}</span>
                    </div>
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:0.75rem; margin-top:1rem; padding-top:1rem; border-top:1px solid var(--slate-100);">
                <div class="rate-box" style="background:#eff6ff;">
                    ...
                    <div class="rate-value" style="color:var(--navy);">{{ $taxa }}%</div>
                </div>
                <div class="rate-box" style="background:#ecfdf5;">
                    <div class="rate-label" style="color:#10b981;">Taxa de Recebimento</div>
                    <div class="rate-value" style="color:#059669;">{{ $taxaRecebimento }}%</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="section-card" style="display:flex; flex-direction:column; justify-content:space-between;">
        <div>
            ...
            <div style="padding:0 1.25rem 1.25rem;">
                <div style="text-align:center; padding:1rem 0;">
                    <div style="font-size:2.5rem; font-weight:700; color:var(--slate-900);">{{ $totalRemanejamentos }}</div>
                ...
                <div style="border-top:1px solid var(--slate-100); padding-top:1rem; text-align:center;">
                    <div style="font-size:1.5rem; font-weight:700; color:#9333ea;">{{ number_format($totalQuantidadeTransferida ?? 0, 0, ',', '.') }}</div>
        ...
        <div style="padding:0 1.25rem 1.25rem;">
            ...
            <div style="display:flex; align-items:center; gap:0.5rem;">
                ...
                <span style="font-size:1.25rem; font-weight:700; color:var(--slate-900);">{{ $divergencias }}</span>
...
<div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem;">
    ...
    <div class="section-card">
        ...
        <div class="rhc-table-wrap">
            <table class="rhc-table">
                ...
                <tbody>
                    @forelse($topItens as $i)
                        <tr>
                            <td style="font-family:ui-monospace,monospace; font-size:0.8rem; color:var(--slate-600);">{{ $i->codigo }}</td>
                            <td style="font-size:0.875rem;">{{ $i->nome }}</td>
                            <td style="text-align:center; font-weight:700; font-size:0.875rem;">{{ number_format($i->total_quantidade, 0, ',', '.') }}</td>
                        </tr>
                    @empty
                        <tr>
    ...
    <div class="section-card">
        ...
        <div style="padding:0 1.25rem 1.25rem;">
            @forelse($pedidosPorUnidade as $pu)
                @php
                ...
                <div class="unit-bar">
                    <div class="unit-row">
                        <span class="unit-name">{{ $pu->nome }}</span>
                        <span class="unit-count">{{ $pu->total }} pedidos</span>
                    </div>
                    <div style="height:0.375rem; border-radius:9999px; background:var(--slate-100); overflow:hidden;">
                        <div style="height:100%; border-radius:9999px; background:var(--navy); width:{{ $percentU }}%; transition:width 0.5s;"></div>
                    </div>
                </div>
            @empty
        ...
        <div style="padding:1.25rem; display:flex; align-items:center; justify-content:space-between;">
            <div style="display:flex; align-items:center; gap:0.75rem;">
                ...
                <div>
                    ...
                    <div style="font-size:1.5rem; font-weight:700; color:var(--slate-900);">R$ {{ number_format($valorTotal, 2, ',', '.') }}</div>
                </div>
            </div>
